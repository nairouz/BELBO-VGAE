...
class BELBO(nn.Module):
    ...
    def train(self, teacher, teacher_ema_updater, features, adj_norm, adj_label, adj, y, weight_tensor, norm, optimizer="Adam", epochs=200, lr=0.01, beta_1=0.3, beta_2=0.15, save_path="./results/", dataset="Cora"):
        ...
        logwriter = csv.DictWriter(logfile, fieldnames=['iter', 'nau', 'ua_0', 'ua_1', 'ua_2', 'ua_3', 'ua_4', 'ua_5', 'ua_6', 
                                                        'ua_7', 'ua_8', 'ua_9', 'ua_10', 'ua_11', 'ua_12', 'ua_13', 'ua_14', 'ua_15',
                                                        'acc', 'nmi', 'ari', 'pur', 'f1_macro', 'f1_micro',
        ...
        for epoch in epoch_bar:
            ...
            with torch.no_grad():
                ...
                p_neg = self.assignment_1(z_mu_neg)
            ...
            ##############################################################
            # Save logs
            logdict = dict(iter=epoch, nau=nau, 
                           ua_0=ua[0].detach().cpu().numpy(),
                           ua_1=ua[1].detach().cpu().numpy(),
                           ua_2=ua[2].detach().cpu().numpy(),
                           ua_3=ua[3].detach().cpu().numpy(),
                           ua_4=ua[4].detach().cpu().numpy(), 
                           ua_5=ua[5].detach().cpu().numpy(),
                           ua_6=ua[6].detach().cpu().numpy(),
                           ua_7=ua[7].detach().cpu().numpy(),
                           ua_8=ua[8].detach().cpu().numpy(),
                           ua_9=ua[9].detach().cpu().numpy(),
                           ua_10=ua[10].detach().cpu().numpy(),
                           ua_11=ua[11].detach().cpu().numpy(),
                           ua_12=ua[12].detach().cpu().numpy(),
                           ua_13=ua[13].detach().cpu().numpy(),
                           ua_14=ua[14].detach().cpu().numpy(),
                           ua_15=ua[15].detach().cpu().numpy(), 
                           acc=acc, nmi=nmi, ari=ari, pur=pur, f1_macro=f1_macro, f1_micro=f1_micro,
